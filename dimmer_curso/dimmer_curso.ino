...
#define lamp1 6
#define led 14
#define ModoDIMMER 15
#define ModoCONTROLE 16
...
void setup() {
  ...
  pinMode(ModoDIMMER, INPUT);
  pinMode(ModoCONTROLE, INPUT);
  digitalWrite(ModoDIMMER, LOW);
  digitalWrite(ModoCONTROLE, HIGH);
  //attachInterrupt(0, zero_cross, FALLING);
}

void loop() {
 while(digitalRead(ModoDIMMER) == HIGH){
  attachInterrupt(0, zero_cross, FALLING);
if(Serial.available()){
  do{
...
while(digitalRead(ModoCONTROLE) == HIGH){
